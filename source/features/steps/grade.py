...
def open_login_page(context):
    context.browser.get('http://localhost:8000/grades/')


@then('Я должен быть на странице "Оценка"')
def should_be_at_list_page(context):
    assert context.browser.current_url == "http://localhost:8000/grades/"


@given('Я перехожу на страницу создания Оценки')
def should_be_at_main(context):
    context.browser.get('http://localhost:8000/grades/add/')
