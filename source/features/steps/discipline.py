...
def open_login_page(context):
    context.browser.get('http://localhost:8000/disciplines/')


@then('Я должен быть на странице "Дисциплина"')
def should_be_at_list_page(context):
    assert context.browser.current_url == "http://localhost:8000/disciplines/"


@given('Я перехожу на страницу создания Дисциплины')
def should_be_at_main(context):
    context.browser.get('http://localhost:8000/disciplines/add/')
